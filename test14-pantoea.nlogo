...
[
  ...
  DST
  density_pa width_big_pa width_small_pa length_big_pa length_small_pa
  ...
  nglucose noxygen namonium nbicarbonate ; Culture medium nutrients availability obtanied from Dab
  mmol_small_pa mmol_big_pa ini_biomass_pa
...
breed [b-donors b-donor] ; e.coli species plasmid donor

breed [b-aceptors b-aceptor] ; bacteria to receive the plasmid from e.coli
...
to setup
  ...
  file-type "# colors-gen " file-print colors-gen
...
to write-outputfile
  ...
  ;file-type "     "
  ;carefully [ file-type (count b-aceptors) ] [ file-type "0" ]
  ;file-type " "
  ;file-print " "
  file-close
end
...
to go
  ...
  ;if (ticks mod 30 = 0) [ write-outputfile ]
  write-outputfile
...
ask b-aceptors
  [
    ...
    metabolism_end_pa
    reproduce_pa
    ;ifelse growth = "non-exponential" [ reproduce_pa ] [
      ;ifelse growth = "exponential"
      ;[ reproduce_pa_exp ] [ reproduce_pa_push ]
    ;]
    ;resize
    ;if growth = "push" [ push ]

      if time-viability_pa >= max-time-viability_pa [set time-viability_pa max-time-viability_pa]
   ; ifelse time-viability_pa < (0.75 * max-time-viability_pa) [ set color cyan ] [ set color cyan ]
...
to setup-thermodynamics-pantoea
 ;--------------------------------------- procedure to setup e-aceptor e-acceptor and microorganims half-reactions matrix -------------------------------
...
; 0 9 ("e.coli")  position for e.coli biomas in matrix reactions
; 0 10 ("∆G")     position for ∆G in matrix reactions

  ;  (rd) Electron aceptor --> Glucose :
 ...
 ;0.1744 CO2 + 0.055 HCO3- + 0.055 NH4+ + 1 H+ + 1 e- --> 0.055 C'4.17'H'8'O'1.75'N'1 + 0.4175 H2O [ dG = 18.3112 KJ/e-eq ]
  ...
  set carbon 4.17 set hydrogen 8 set oxygen 1.75 set b-nitrogen 1                      ; patoea empirical formula C4.17H8O1.75N
  set molecular-weight 12.011 * carbon + 1.008 * hydrogen + 16 * oxygen + 14 * b-nitrogen ; molecular weight of e.coli empiral formula
  ...
  set dGpc (3.324 * molecular-weight / e-eeq/mol)
   ...
   matrix:set rc 0 9 (-1 * b-bio) ; e.coli
...
; ---- Thermodynamic calculations to predict metabolic pathways using TEEM1 ---------------------------------------------------------------------
 ...
 set energy_reaction ra matrix:- rd ; matrix with the stoichimetric coefficients to simulated celullar maintenance for e.coli
...
to setup-medium
 set Glu random-normal (Glucose) (DST * Glucose)           ; Succinate initial concentration (user define using a slider labeled Succinate) [mM]
 set Amo random-normal (Amonium) (DST * Amonium)               ; Amonium initial concentration (user define using a slider labeled Amonium) [mM]
 set Oxy random-normal  (100) (0.1 * 100) ;  (0.2360) (DST * 0.2360)                 ; Oxygen initial concentration (simulated saturated conditions) accord to Felgate's (2012) experiment [mM]
 set Bic random-normal (1000) (DST * 1000)               ; Nitrate initial concentration (user define using a slider labeled Nitrate) [mM]
 ask patches
   [
     set glucose-medium ((abs (random-normal (Glu) (DST * Glu)) * world) / (world-width * world-height)) ; Assign the succinate concentration to each cell according to the model's standar deviation
     set oxygen-medium ((abs (random-normal (Oxy) (DST * Oxy)) * world) / (world-width * world-height))              ; Assign the amonium concentration to each cell according to the model's standar deviation
     set amonium-medium  ((abs (random-normal (Amo) (DST * Amo)) * world) / (world-width * world-height))             ; Assign the oxygen concentration to each cell according to the model's standar deviation
     set bicarbonate-medium ((abs (random-normal (Bic) (DST * Bic)) * world) / (world-width * world-height))              ; Assign the nitrate concentration to each cell according to the model's standar deviation
     set dioxide 0                                                ; Nitric Oxide initial concentration accord to Felgate's (2012) experiment [mM]
     set water 0                                                       ; Nitrogen gas (N2) initial concentration accord to Felgate's (2012) experiment [mM]
     ;set pcolor scale-color black glucose-medium 0 (2 * Glu)
   ]

end

to setup-pantoea
 let gen-list n-values (Microrganism * 2)  [i -> i]
  create-b-aceptors Microrganism                                                   ; User define the amount of initial active bacteria using a slider labaled Microrganism
   [

   set gen-pa first gen-list + 1
    set gen-list butfirst gen-list

   set shape "bacteria"
   set color green         set heading (random 360)                                                   ; This color is for visual propouse only
   ...
   set pa-biomass-reproduction abs random-normal (ini_biomass_pa) (DST * ini_biomass_pa)
    set repr_time_pa 0; abs random-normal (rep_pa) (DST * rep_pa) ; Assuming seed in exponential growth behavior or <0> if is lag behavior
  ]
end
...
to setup-monitors
  ...
  set gglucose (sum [glucose-medium]of patches / world)                                               ; Succinate concentration [ mM ]
  ...
  set gbicarbonate (sum [bicarbonate-medium] of patches / world)
  set gdioxide (sum [dioxide] of patches / world)
  set gwater (sum [water] of patches / world)                                                 ; Nitrate concentration [ mM ]

  reactor_balance
end

to push
  ask other turtles in-radius ( (pa-biomass-reproduction * molecular-weight / density_pa) ^ .33333 * 2 + size) [ ;only has to check for turtles nearby
    let orig-ori heading
    let d distance myself
    let sum-radii size / 2 + [ size ] of myself / 2
    if d < sum-radii
    [
      face myself
      fd d - sum-radii
      set heading orig-ori
    ]
  ]
end
...
to color-nutrient
  set pcolor black ;scale-color black glucose-medium 0 (2 * Glu)
end
...
to reactor_balance
  ...
  set fb-aceptors (gbacteria_pa * 1000 * world / molecular-weight)              ; Initial Biomass [mmol]
  set fglucose (gglucose * world)                                        ; Initial Glucose [mmol]
  set famonium (gamonium * world)                                           ; Initial Amonium [mmol]
  set foxygen (goxygen * world)                                             ; Initial Oxygen [mmol]
  set fwater (gwater * world)                                     ; Initial water [mmol]
  set fdioxide (gdioxide * world)                                                  ; Initial Carbon dioxide [mmol]
  set fbicarbonate (gbicarbonate * world)
...
Version for Pantoea growth in agar plate. Experimental conditions for nutrients concentration and size of the world were considered.
